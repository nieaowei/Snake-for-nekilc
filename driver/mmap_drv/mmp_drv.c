...
#define PAGE_SIZE_TEMP (800*480*4)

//lcd信息
struct fb_fix_screeninfo finfo;
struct fb_var_screeninfo vinfo;
unsigned int page_size_temp = 0;//lcd屏幕占用的空间
...
static int ubuntu_lcd_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	addr = virt_to_phys(virt_mem);
 	if (io_remap_pfn_range(vma, vma->vm_start, addr >> PAGE_SHIFT,
					PAGE_SIZE_TEMP, vma->vm_page_prot)) {
...
static int __init ubuntu_lcd_init(void)
{
    ...
    }else
    {
		...
		page_size_temp = vinfo.xres*vinfo.yres*vinfo.bits_per_pixel/8;
		virt_mem = kzalloc(page_size_temp,  GFP_KERNEL);
